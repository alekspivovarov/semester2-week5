...
int main(void) {
    ...
    do { 
        ...
        if (result != 2) {
    // try with degree symbol (skip 2 bytes)
    char skip[3];
    result = sscanf(input, "%f%2c%c", &temperature, skip, &scale);
}
...
// TODO: Validate the scale (must be 'C', 'c', 'F', or 'f')
        ...
        if (result != 2) {
    printf("Error: invalid format. Use e.g. 23.5C or 75F\n"); 
    } else {  scale = toupper(scale);
        if (scale == 'C' || scale == 'F') {
            valid_input = 1;
        } else {
